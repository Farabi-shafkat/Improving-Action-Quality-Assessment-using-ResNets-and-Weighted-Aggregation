...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        conv1_t_size_ch = 3
       # print("lokkkkkkkk at meee",conv1_t_size)
        block_inplanes = [int(x * widen_factor) for x in block_inplanes]

        self.in_planes = block_inplanes[0]
        self.no_max_pool = no_max_pool
      
        n_3d_parameters = 3 * self.in_planes * conv1_t_size_ch * 7 * 7
        n_2p1d_parameters = 3 * 7 * 7 + conv1_t_size_ch * self.in_planes
        mid_planes = 45 #n_3d_parameters // n_2p1d_parameters
        ...
        self.conv1_t = nn.Conv3d(mid_planes,
                                 self.in_planes,
                                 kernel_size=(conv1_t_size_ch, 1, 1),
                                 stride=(conv1_t_stride, 1, 1),
                                 padding=(conv1_t_size_ch // 2, 0, 0),
                                 bias=False)
...
def build_model(depth,pretrained = True):
    if pretrained == True:
    ...
    model = generate_model(model_depth=depth,
                            ...
                            widen_factor=1.0)
    if pretrained: 
        pretrained_dict = torch.load(os.path.join(pretrained_weights_dir,'r2p1d{}_K_200ep.pth'.format(depth)))['state_dict']
        model_state_dict = model.state_dict()
        ig_model_state_dict = torch.hub.load("moabitcoin/ig65m-pytorch", "r2plus1d_34_32_kinetics", num_classes=400, pretrained=True).state_dict()
 

        for key in model_state_dict.keys():
            print(key,"..")
            if model_state_dict[key].shape == pretrained_dict[key].shape:
                model_state_dict[key] = pretrained_dict[key]
                print(key)
            else:
                modified_key = list(ig_model_state_dict.keys())[list(model_state_dict.keys()).index(key)]
                print(key," -> ",modified_key)
                model_state_dict[key] = ig_model_state_dict[modified_key]
